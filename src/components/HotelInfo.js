...
const HotelInfo = () => {
  ...
  const loadAccessibilityData = async() => {
    ...
    const resp = await fetch('https://4w8p555qy7.execute-api.eu-central-1.amazonaws.com/Production/accessibilities');
  ...
  const loadServicesData = async() => {
    // Query the API Gateway
    const resp = await fetch('https://4w8p555qy7.execute-api.eu-central-1.amazonaws.com/Production/services');
    let jsonData = await resp.json();
